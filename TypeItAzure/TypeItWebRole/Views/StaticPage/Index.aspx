...
#grid {
  ...
  <body>
    ...
    <script>
        ...
        var server = "http://typeit.cloudapp.net";

        if (useTestServer) {
            server = "http://localhost:19500"
        }

        $.get(server + "/Home/getuserdata?UserName=Jenny", function (data) {
        });

        $.get(server + "/Home/getstats?UserName=Jenny", function (data) {
        });

        $.get(server + "/Home/getwords?UserName=Jenny", function (data) {
        });
        ...
        function submitSettings() {
            ...
            $.get(server + "/Home/updateuserdata?UserName=Jenny&" + args, function (data) {
                alert('User Updated!');
            });
        }
        ...
        function jsonAvailableWordsCallback(data) {
            document.getElementById('loading2').style.display = "none";

            //this will return a list
            $.each(data, function (key, value) {

                var oRow = document.getElementById('availableTable').insertRow(-1);
      ...
      <hr />
        <section style="width:640px;margin-left:30px">
        <h2>Settings</h2>
        ...
        <button type="button" style="width:200px;margin-top:20px;margin-left:auto;margin-right:auto;" id="submitSettings" onclick="submitSettings()">Save Settings</button>
     </section>
    <br />
    ...
    <section style="margin-left:30px">
        ...
        <h3 id="loading2" style="color:red">Loading....</h3>
        <table id="availableTable" style="border: 2px solid black;">
            <thead><tr>
            <th >Word</th>
            <th style="background-color:lightblue">Image Link</th>
            <th>Reward Link</th>
            </tr>
             </thead>
       </table>
